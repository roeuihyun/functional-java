...
public class AdvancedStream1 {
	
	public static void main(String[] args) {
		
//		Optional<Integer> max = Stream.of(5,3,6,2,1).max((x,y)-> x-y);
//		System.out.println("max : " + max);
		Optional<Integer> max = Stream.of(5,3,6,2,1).max(Integer::compareTo);
		...
		User firstUser = users.stream()
				.min((u1,u2)-> u1.getName().compareTo(u2.getName())).get();
		System.out.println("firstUser : " +firstUser);
		
		long postiveIntegerCount = Stream.of(1, -4, 5, -3, 6)
			.filter(x -> x > 0)
			.count();
		System.out.println("postiveIntegerCount : " + postiveIntegerCount);
		...
		long isUnVerifiedUserCount = users.stream()
			.filter(user -> user.getCreatedAt().isAfter(now.minusDays(1)))
			.filter(user -> !user.isVerified())
			.count();
		System.out.println("isUnVerifiedUserCount : " + isUnVerifiedUserCount);
